...
class _NoteListScreenState extends State<NoteListScreen> {
  ...
  String _dateFormat = 'GG.AA.YYYY SS:DD';
  int _previewLines = 3;
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      ...
      final bgColorVal = prefs.getInt('bg_color');
      ...
      _dateFormat = prefs.getString('date_format') ?? 'GG.AA.YYYY SS:DD';
  ...
  Future<void> _saveData() async {
    ...
    await prefs.setInt('bg_color', _bgColor.value);
          ...
          child: ListView(
            ...
            children: [
              ...
              ListTile(
                ...
                selected: _activeCategory == 'Tümü' || _activeCategory == 'Notlar',
              ...
              ListTile(
                ...
                selected: _activeCategory == '__favorites__',
                selectedTileColor: Colors.amber.withOpacity(0.1),
                onTap: () {
              ...
              ListTile(
                ...
                selected: _activeCategory == '__locked__',
                selectedTileColor: Colors.amber.withOpacity(0.1),
              ...
              ListTile(
                ...
                selected: _activeCategory == '__archive__',
                selectedTileColor: Colors.amber.withOpacity(0.1),
                onTap: () {
              ...
              ListTile(
                ...
                selected: _activeCategory == '__trash__',
                selectedTileColor: Colors.amber.withOpacity(0.1),
                onTap: () {
  ...
  Widget _buildGridView({
    ...
    for (int index = 0; index < filteredNotes.length; index++) {
      ...
      final estimatedLines = _estimateNoteLines(note);
  ...
  // Kartın yaklaşık satır sayısını tahmin eder (sütun dengelemesi için).
  // _previewLines ile sınırlandırılır, böylece bir notun çok uzun olması
  // sütun dengesini bozmaz; gerçek görsel sınır Text widget'ındaki
  // maxLines tarafından zaten uygulanır.
  int _estimateNoteLines(Map<String, dynamic> note) {
    final hasTitle = (note['title'] ?? '').toString().isNotEmpty;
    final isChecklist = note['type'] == 'checklist';
    int lines = hasTitle ? 1 : 0;

    if (isChecklist) {
      final itemCount = (note['checkItems'] as List? ?? []).length;
      lines += itemCount.clamp(0, _previewLines);
    } else {
      final content = (note['content'] ?? '').toString();
      final contentLineCount =
          content.isEmpty ? 0 : content.split('\n').length;
      lines += contentLineCount.clamp(0, _previewLines);
    }
                  ...
                  children: [
                    ...
                    isChecklist
                        ...
                        : Text(
                            note['content'] ?? '',
                            style: TextStyle(
                                color: _textColor.withOpacity(0.7), fontSize: 13),
                            maxLines: _previewLines,
                            overflow: TextOverflow.ellipsis,
                          ),
                  ],
...
class _SettingsPageState extends State<_SettingsPage> {
  ...
  // ── Arka plan rengi seçici ─────────────────────────────────────────
  static const List<Color> _bgPalette = [
    Color(0xFF121212), Color(0xFF1A1A2E), Color(0xFF0D1117),
    Color(0xFF1C1C1E), Color(0xFF0A0A0A), Color(0xFF1B2631),
    Color(0xFF1A1200), Color(0xFF0D1B0D), Color(0xFF1A0010),
  ];
            ...
            child: Column(
              ...
              children: [
                Container(width: 40, height: 4, margin: const EdgeInsets.only(bottom: 16, left: 120, right: 120),
                  decoration: BoxDecoration(color: Colors.grey[700], borderRadius: BorderRadius.circular(2))),
                const Text('Arka Plan Rengi', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                const Text('Uygulama arka planı için renk seçin.', style: TextStyle(color: Colors.grey, fontSize: 12)),
                const SizedBox(height: 20),
                Wrap(
                  spacing: 12,
                  runSpacing: 12,
                  children: _bgPalette.map((c) {
                    final selected = s._bgColor == c;
                    return GestureDetector(
                      onTap: () {
                        s.setState(() => s._bgColor = c);
                        setState(() {});
                        s._saveData();
                      },
                      child: Container(
                        ...
                        child: selected ? const Icon(Icons.check, color: Colors.amber, size: 20) : null,
                      ),
                    );
                  }).toList(),
                ),
            ...
            child: Column(
              children: [
                ...
                _settingTile(
                  ...
                  subtitle: 'Uygulama arka plan rengini özelleştir.',
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Container(
                      ...
                      builder: (_) => SafeArea(
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Tarih Biçimi', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                              const SizedBox(height: 4),
                              const Text('Not kartlarında görünen tarih biçimi.', style: TextStyle(color: Colors.grey, fontSize: 12)),
                              const SizedBox(height: 12),
                              ..._dateFormats.map((f) => ListTile(
                                contentPadding: EdgeInsets.zero,
                                title: Text(f, style: TextStyle(color: s._dateFormat == f ? Colors.amber : Colors.white)),
                                trailing: s._dateFormat == f ? const Icon(Icons.check_circle, color: Colors.amber) : null,
                                onTap: () {
                                  s.setState(() => s._dateFormat = f);
                                  setState(() {});
                                  s._saveData();
                                  Navigator.pop(context);
                                },
